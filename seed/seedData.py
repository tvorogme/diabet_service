...
while now_date < today:
    ...
    print(now_date.strftime("%Y-%m-%dT%H:%M:%S.000Z"))
    if random.randint(1, 7) == 1:
        db['results'].insert_one({
            "value": random.randint(10, 40),
            "time": now_date.strftime("%Y-%m-%d %H:%M:%S"), "otime": now_date,
            "type": "ID",
            "user_id": name
        })
        continue
...
while date_copy < today:
    ...
    db['results'].insert_one({
        ...
        "time": date_copy.strftime("%Y-%m-%d %H:%M:%S"), "otime": date_copy,
        "type": "WE",
        "user_id": name,
        "name": 'Вес'
    })
...
now_date = datetime.datetime(2017, 3, 1, 0, 0);
